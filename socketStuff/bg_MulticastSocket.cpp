...
bool bg_MulticastSocket::Intialize(const char* multicastGroupString, int multicastPort, unsigned short loopBack, unsigned short ttl, std::function<void(char* message, int messageSize)> onReceive)
{
    ...
    */
    ...
    {
        printf("setsockopt() failed for socket reuse with error %d\r\n", WSAGetLastError());
        CleanUp();
        return false;
    }
...
bool bg_MulticastSocket::LeaveGroup()
{
...
bool bg_MulticastSocket::Send(char* message, int messageSize, int& bytesSent)
{
    // Return if socket is not created.
    if (m_isSocketCreated == false)  return false;
